...
<template>
	<div>
		...
		<div class="_spacer" style="--MI_SPACER-min: 20px; --MI_SPACER-max: 32px;">
			<form :key="captchaKey" class="_gaps_m" autocomplete="new-password" @submit.prevent="onSubmit">
				...
				<MkInput v-if="instance.emailRequiredForSignup || instance.requireApplicationForSignup" v-model="email"
					:debounce="true" type="email" :spellcheck="false" required data-cy-signup-email
...
const usernameAbortController = ref<AbortController | null>(null);
const emailAbortController = ref<AbortController | null>(null);

const captchaKey = computed(() => {
	if (instance.enableHcaptcha) return 'h';
	if (instance.enableMcaptcha) return 'm';
	if (instance.enableRecaptcha) return 'r';
	if (instance.enableTurnstile) return 't';
	if (instance.enableTestcaptcha) return 'test';
	return 'none';
});
...
function onChangeUsername(): void {
	...
	if (usernameAbortController.value) usernameAbortController.value.abort();
	...
	misskeyApi('username/available', { username: username.value }, undefined, usernameAbortController.value.signal).then(result => {
		...
		if (err.name !== 'AbortError') usernameState.value = 'error';
	});
}
...
function onChangeEmail(): void {
	...
	if (emailAbortController.value) emailAbortController.value.abort();
	...
	misskeyApi('email-address/available', { emailAddress: email.value }, undefined, emailAbortController.value.signal).then(result => {
		...
		if (err.name !== 'AbortError') emailState.value = 'error';
	});
}
...
async function onSubmit(): Promise<void> {
	...
	if (res && res.ok) {
		if (res.status === 204) {
			if (instance.requireApplicationForSignup) {
				os.alert({
					type: 'success',
					title: '申請を送信しました',
					text: '管理者の承認をお待ちください。',
