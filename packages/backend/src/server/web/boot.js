...
(async () => {
	// 常に利用できるよう、renderErrorとaddStyleを先頭に移動
	async function addStyle(styleText) {
		// systemdが初期化されていない可能性を考慮
	...
	async function renderError(code, details) {
		// systemdがまだ初期化されていない場合があるため、存在すれば呼び出す
		if (typeof systemd !== 'undefined') {
	...
	const systemd = new Systemd(typeof VERSION !== 'undefined' ? VERSION : 'dev', cmdline);
	...
	async function importAppScript() {
		// upstreamの言語置換ロジックを復元
		const entryPoint = CLIENT_ENTRY ? `/vite/${CLIENT_ENTRY.replace('scripts', lang)}` : '/vite/src/_boot_.ts';
		await systemd.start('Load App Script', import(entryPoint))
			.catch(async e => {
				console.error(e);
	...
	if (document.readyState !== 'loading') {
		await systemd.start('import App Script', importAppScript());
	} else {
